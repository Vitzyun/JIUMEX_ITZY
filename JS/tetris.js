...
(function () {
  ...
  Grid.prototype = {
    ...
    findCompletedRows: function (piecePosition) {
      ...
      var score =
        Object.keys(removeRows).length * Object.keys(removeRows).length;
      this.score += score;
      this.outputScore();
  ...
  Piece.prototype = {
    ...
    displayPreviewPiece: function (previewCells) {
      ...
      for (var index in this.currentPosition) {
        ...
        previewCells[coordinates.y + 1][coordinates.x - 3].markCell(
          this.colour
        );
      }
    },
    ...
    setAllowedMoves: function (cells) {
      ...
      for (var index in this.currentPosition) {
        ...
        if (
          Config.size.width - 1 === coordinates.x ||
          1 === cells[coordinates.y][coordinates.x + 1].state
        ) {
        ...
        if (
          Config.size.height - 1 === coordinates.y ||
    ...
    devOutput: function () {
      ...
      for (var coords in this.currentPosition) {
        coordinates +=
          c +
          "y:" +
          this.currentPosition[coords].y +
          " x:" +
          this.currentPosition[coords].x +
          "; ";
...
(function () {
  ...
  Cell.prototype = {
    ...
    addDevOutput: function () {
      if (true === this.devState) {
        this.cellHtml.innerHTML =
          "<span id='js-x-dev' class='cell-dev x-dev'>" +
          this.x +
          "</span><span id='js-y-dev y-dev' class='cell-dev'>" +
          this.y +
          "</span><span id='js-state-dev-" +
          this.x +
          "-" +
          this.y +
          "' class='cell-dev state-dev'>" +
          this.state +
          "</span>";
      }
    }
